...
| 主設定 | `portable-city.yml`, `server.yml`, `progression-campaigns.yml`, `community-projects.yml`, `packet-quest-boards.yml` |
...
city 本体の挙動や cross-server 転送情報は `portable-city.yml` にあり、`/hub` `/pve` の候補リストだけは `server.yml` の `routing.commands` で管理します。
...
3. 公開 city なら `/city visit` で非メンバーも見学に入れる
4. spawn 周辺は絶対安全地帯として案内や集合地点に使える
5. 週替わり campaign や project tier が長期目標を提示する
6. party 招待で複数人行動に入り、questboard から自分向け候補へつながる
7. board 右クリックか受付 NPC クリックで詳細確認し、/yes /no で受注を決める
...
- preview は packet block change で本人にだけ見えるため、他プレイヤーの視界や実 block を汚さない
- player が歩く、向きを変える、見る面を変えると preview も追従して更新される
- Shift+右クリックでモード切替、Shift+左クリックでサイズ切替、左クリックで現在状態確認、右クリックで確定設置
- spawn safe radius 内や visitor 状態では preview も実設置も止まる
...
1 group の上限は 5 city なので、この系列を上限まで使います。
