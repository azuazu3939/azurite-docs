# ポータブルシティ・キャンペーン・パーティー・クエスト看板

日常導線と長期目標をまとめる運用側の要素です。  
city、campaign、community project、party、questboard がつながり、プレイヤー個人の進行をサーバー全体の流れへ接続します。

## この要素が担うこと
- city を拠点として安全地帯、訪問、移動の基準を作る
- campaign や project で週次・長期の目標を供給する
- party と questboard で「次に何をするか」を迷いにくくする
- party をサーバー間でも維持し、必要な時だけリーダーのサーバーへ集合しやすくする

## プレイヤーから見る流れ
- city を起点に日々の行動を始める
- questboard や campaign で今週の目的を選ぶ
- party や community project を通じて個人進行を協力プレイへ広げる

## つながる要素
- questboard は Frontier、戦闘、採集、経済導線の入口になる
- campaign 報酬や project 進行は成長、制作、流通全体に波及する
- 都市運用や掲示板定義の詳細は [設定項目](./reference.md) を参照

## 関連
- [設定項目](./reference.md)
- [編集例](./examples.md)
- [Wiki](./wiki.md)
