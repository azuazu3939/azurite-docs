...
| 項目 | 内容 |
| --- | --- |
| 主役 | Vault、共有倉庫、ドロップ倉庫、BackPack、直売、Market Board、Trade Shop |
...
共有倉庫、ドロップ倉庫、BackPack、直売、マーケット、Trade Shop は別機能に見えますが、runtime 内では受取、ledger、価格体感が強くつながっています。  
...
- `/backpack bind` で手持ちアイテムへ `/bp` ショートカットをバインドできる
...
- `よく使うコマンド集` は `menu`、`BackPack`、`MySet` を含む最新一覧を表示します。
- `BackPack` は `/backpack (/bp) [bind]`、`MySet` は `/myset [list|save|load|delete]` として案内します。
