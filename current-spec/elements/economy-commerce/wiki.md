...
- Trade Shop では商品スロットにカーソルを合わせてドロップキーを押すと、その商品を直接目標化して GUI を閉じ、黄色ガラス案内へすぐ移れます。
- Shop 由来の素材は、Shop 窓口そのものよりも、選ばれた工程の子素材や MythicMob Drop を深く逆探索する方を優先します。
- 途中素材を一部だけ持っている場合も、そこで止めず、不足分を作るための素材分岐をすべて辿り、未減算の必要素材を最後に同じ素材ごとへ合算してから所持数を差し引いて表示します。
...
- `専門職` は `/profession`、`/profession list`、`/profession info <profession>`、`/profession top <profession> [count]` を案内し、採集・Packet 釣り・錬金の職業進行と職業別ランキングへ戻れる入口にします。
