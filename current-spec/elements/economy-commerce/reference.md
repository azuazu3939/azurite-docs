...
| UI / コマンド | 役割 | 変更時の見方 |
| --- | --- | --- |
| `/menu` | 自分用メニュー GUI を開く。 | 初期配置はテンプレートで、通常クリックは実行、`Shift+右クリック` だけショートカット化する。 |
...
| `しゃがみ右クリック` | 手持ちのバインド済みアイテムを消去する。 | 一般プレイヤー向けの基本解除操作。 |
| `/unbind` | 手持ちのバインド済みアイテムを消去する。 | クリックで消せない状況向けの解除入口。 |
| `/bind clear` | 手持ちのバインド済みアイテムを消去する。 | 旧導線との互換。固定ショートカットは解除できない。 |
...
| コマンド集の一覧 | 一般プレイヤーが普段使う前提のコマンドだけを案内する。 | `menu` は自分用 GUI、`BackPack` は `/backpack (/bp) [1-6\|bind [1-6]]`、`MySet` は `/myset [list\|save\|load\|delete]` として出す。解除は基本 しゃがみ右クリックで消去。 |
