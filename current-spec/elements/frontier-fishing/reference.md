...
| `species.[species-id].reward-exp` | 撃破時 exp。 | 釣り育成速度。 |
...
| `routes/*.yml` | route 定義。 | `id` `label` `profile-id` `family` `domain` `spawn-profile` `entry-offsets` `progression-band` `access-tags` を持ち、`entry-offsets` は mining-entry や遠征の入口候補に使う。 |
