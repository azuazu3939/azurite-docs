...
2. territory に応じて出現位置や world boss 側の報酬倍率が変わる
...
| territory 計算 | `FrontierTerritoryService` | chunk ごとの所有者、spawn 条件、world boss 報酬倍率 |
...
いいえ。通常の world generator mob へ直接補正はかけませんが、出現分布と world boss 側の報酬倍率には関わります。
