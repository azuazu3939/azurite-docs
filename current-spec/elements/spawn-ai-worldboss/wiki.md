...
| 主設定 | `mob-spawns/*.yml`, `mythic-ai.yml`, `mythic-ai-feedback.yml` |
...
- 現在は `JOIN`、`CHAT`、`GATHER_BREAK`、`MYTHIC_KILL` に加えて、繰り返し詰まりを拾う `PROGRESSION_STUCK` を対象に、`serverId` と `worldId` つきで `telemetry/agent-state/<server-id>/latest.json` と `telemetry/agent-inbox/<server-id>/...` へ出力します。
