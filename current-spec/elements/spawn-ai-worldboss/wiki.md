...
## 2026-04 の player telemetry 追加

- MariaDB に `ai_player_activity_profile` と `ai_player_activity_event` を追加し、player ごとの最近傾向と直近イベントを残すようになりました。
- 現在は `JOIN`、`CHAT`、`GATHER_BREAK`、`MYTHIC_KILL` を対象に、`serverId` と `worldId` つきで保存します。
- 「何と戦っているか」「何を採っているか」「chat で何を言ったか」を後から追えるため、Cardinal 系の自律調整や観測追加判断の入口になります。
