...
| 主設定 | `mob-spawns/*.yml`, `mythic-ai.yml`, `mythic-ai-feedback.yml`, `mythic-ai-drive.yml` |
...
| Feedback export | `core/src/main/resources/mythic-ai-feedback.yml` | feedback Git へ JSON を出す周期と保持設定 |
| Drive 同期 | `core/src/main/resources/mythic-ai-drive.yml` | policy import / export を使う時だけ有効化 |

## 2026-04 の player telemetry / feedback 変更

- 長期の agent 観測は MariaDB 直書きではなく、`feedback-mythicmobs` repo の JSON を主系統に寄せました。
- 現在は `JOIN`、`CHAT`、`GATHER_BREAK`、`MYTHIC_KILL` を対象に、`serverId` と `worldId` つきで `telemetry/agent-state/<server-id>/latest.json` と `telemetry/agent-inbox/<server-id>/...` へ出力します。
- これにより OpenClaw が無い server PC でも、plugin 単体で観測 JSON を Git 経由でこの PC へ渡せます。
- MariaDB / Redis は runtime の補助依存として残りますが、Cardinal 系の長期判断材料は feedback Git 側を優先します。

## 編集フロー

1. まず「出現条件」「territory」「AI policy」のどこを変えるか決める
2. AI policy だけでなく spawn 条件側も見る
3. feedback export を使うなら `mythic-ai-feedback.yml` と feedback repo の Git 導線を先に確認する
4. Drive 同期を使うなら `mythic-ai-drive.yml` の serverId と認証情報を追加確認する
...
### feedback Git で観測を受け渡したい

- `mythic-ai-feedback.yml` を確認する
- server ごとに `feedback-mythicmobs/telemetry/agent-state/<server-id>/latest.json` が更新される
- 履歴は `telemetry/agent-inbox/<server-id>/yyyy/MM/dd/` へ出る
- retention で古い inbox JSON を落としていく前提なので、長期保管は Git history か集約レポート側で扱う
...
Cardinal 系の長期観測は feedback Git の JSON を優先します。MariaDB / Redis は補助の runtime 状態確認として扱います。
